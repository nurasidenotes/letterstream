...
template = "resources/templates/LetterForwardingTemplate.docx"
...
while True:
    ...
    if event == '-SET MAILTYPE-':
        ...
        if not mail_type_input:
            ...
            company_name, reference, current_date = file_stem.split(' - ')[1:4]
            ...
            output_path = f'temp/{company_name}/'
            if not os.path.exists(docx_folder_name):
                os.makedirs(f'{output_path}{docx_folder_name}')
            ...
            output_csv = f'{output_path}{folder_name}\{company_name}_Batch{order_num:0>3}_{current_date}.csv'
            #output_csv = f"temp\{company_name}\{folder_name}\{company_name}_Batch{order_num:0>3}_{current_date}.csv"
            ...
            with open(csv_file, newline='') as csvfile, open(output_csv, 'w', encoding='UTF8', newline='') as batch_output:
                ...
                for row in csvreader: 
                    ...
                    if not row[headers.index(' Address 1')]:
                        ...
                        id_count += 1
                    document.write(f"{output_path}{docx_folder_name}\{doc_name}")
                ...
                window.refresh()
                convert(f"{output_path}{docx_folder_name}/",f"{output_path}{folder_name}/")
                window.refresh()
            ...
            ## imwatchingyou.refresh_debugger()
            # zips folder
            zip_file = f'{output_path}{folder_name}.zip'
            zip_directory = pathlib.Path(f'{output_path}{folder_name}/')
