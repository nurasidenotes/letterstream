...
template = "src/template_smith_downey.docx"
...
while True:
    ...
    if event == '-RUN-':  
        ...
        if not mail_type_input:
            ...
            docx_folder_name = f"Smith&Downey_{current_date}_Batch{order_num:0>3}_'docx'"
            folder_name = f"Smith&Downey_{current_date}_Batch{order_num:0>3}"

            # To create variable letter template from Company information
            contact_csv = 'resources/db/CompanyContacts.csv'
            # offsite test:
            ...
            with open('resources/templates/Batch_Template.csv', newline='') as output_csv:
                output_reader = csv.DictReader(output_csv)
                output_csv_header = output_reader.fieldnames
                    
            # creates output csv variable object
            output_csv = f'{sd_output_path}{folder_name}\S&D_Batch{order_num:0>3}_{current_date}.csv'
            ...
            def check_name(row):
                no_char_first = re.sub('[^a-zA-Z0-9-]+', '', row[first_name_index])
                no_char_last = re.sub('[^a-zA-Z0-9-]+', '', row[last_name_index])
                return f'{no_char_last}_{no_char_first}'

            def uppercase(merge):
                first = merge[first_name_index]
                last = merge[last_name_index]
            ...
            with open(csv_file, newline='') as csvfile, open(output_csv, 'w', encoding='UTF8', newline='') as batch_output:
                ...
                for row in csvreader: 
                    ...
                    if not row[headers.index(' Address 1')]:
                        ...
                        doc_safe_name = check_name(row)
                        doc_id = f"{current_date}_{initial_stem}{id_count:0>4}"
                        pdf_name = f'{order_count:0>4}_{doc_safe_name}.pdf'
                        doc_name = f'{order_count:0>4}_{doc_safe_name}.docx'
                        full_name = uppercase(row)
                        zip_code = check_zip(row, zip_index)
